...
activate_party='--activate-party' in sys.argv[3:]
...
out.joinpath('before_relocation.state').write_bytes(state)
ram=(C.c_ubyte*131072).from_address(core.retro_get_memory_data(2));old=list(ram[0x200:0x202]);before=bytes(ram)
for a in (0x200,0xA3,0xA6):ram[a]=x;ram[a+1]=y
if move_party:
    for a,dx,dy in ((0x240,-2,0),(0x280,-1,0),(0x2C0,-2,1)):
        ram[a]=(x+dx)&255;ram[a+1]=(y+dy)&127
if activate_party:
    # Test-state unit participation only, for isolated battle QA after travel.
    # Town members otherwise retain an inactive field flag after relocation.
...
with out.joinpath('field_relocations.jsonl').open('a',encoding='utf-8') as f:f.write(json.dumps({'from':old,'to':[x,y],'move_party':move_party,'activate_party':activate_party,'changed_ram_offsets':[hex(i) for i in changed],'rom_sha256':hashlib.sha256((root/'Albert Odyssey - Korean Full.sfc').read_bytes()).hexdigest(),'scope':'QA field travel and optional unit participation; no story flags or inventory changed'})+'\n')
