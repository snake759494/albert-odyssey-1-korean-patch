...
out=root/'analysis_font_scan/ending_review'/('transition_'+kind);out.mkdir(exist_ok=True)
...
b=(root/'analysis_font_scan/ending_review/probe_korean/frame0038.state').read_bytes();buf=C.create_string_buffer(b);assert core.retro_unserialize(buf,len(b))
f=0
@ns['INPUT']
def inp(port,device,index,ident):return int(port==0 and ident==8 and f<12)
core.retro_set_input_state(inp)
for f in range(1200):core.retro_run()
