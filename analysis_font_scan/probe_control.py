...
out=root/'analysis_font_scan/ending_review'/('control_'+kind);out.mkdir(exist_ok=True)
...
f=0
@ns['INPUT']
def inp(port,device,index,ident):return int(port==0 and ((ident==7 and f<4) or (ident==8 and 400<=f<440)))
