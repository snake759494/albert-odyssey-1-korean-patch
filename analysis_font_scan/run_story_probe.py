...
END_FRAME=int(sys.argv[3]) if len(sys.argv)>3 else 20000
...
def inp(port,device,index,ident):
 if port!=0:return 0
 pulses=[(1100,1110,3),(1300,1310,8)]
 # Advance each dialogue/menu prompt after the previous A press has been released.
 pulses += [(f,f+12,8) for f in range(1800,END_FRAME+1,700)]
 return int(any(a<=frame<b and ident==button for a,b,button in pulses))
...
for frame in range(END_FRAME+1):
 core.retro_run()
 if frame >= 1000 and frame % 500 == 0:capture();print('captured',frame,flush=True)
